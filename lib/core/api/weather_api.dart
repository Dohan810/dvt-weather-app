...
class WeatherApi {
  ...
  Future<WeatherModel> getWeather(double lat, double lon) async {
    ...
    try {
      final request = await client.getUrl(Uri.parse('$_baseUrl/weather?lat=$lat&lon=$lon&appid=$_apiKey'));
  ...
  Future<List<ForecastModel>> getForecast(double lat, double lon) async {
    final client = HttpClient();
    try {
      final request = await client.getUrl(Uri.parse('$_baseUrl/forecast?lat=$lat&lon=$lon&appid=$_apiKey'));
      final response = await request.close();
      if (response.statusCode == 200) {
        final responseBody = await response.transform(utf8.decoder).join();
